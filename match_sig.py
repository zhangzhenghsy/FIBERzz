...
#This program aims to match a signature in a target binary.
#try to find each signature in target binary one by one 
#given signature there may be several candidates(topology isomorphisms) in the target binary. generate the candidate using graph_match()
#given candidate, there may be several nodes(basic block). Compare node between signature and candidate one by one. Node mapping is generated by graph_match()
#given node, there may be several root instructions. We need to classify them in several sets (done in classify_formulas()) and compare instructions in each type. 
#Notice that in classify_formulas, the load/other instructions are stored in sets as ASTs(each AST is part of an instruction). The store instructions is stored as addr-data-length tuple(each tuple represent an instruction).The exit instructions is stored as addr-guard-kind tuple
#However, for store instruction tuple, we only retain addr and data._formula_match_store().for exit instruction, we only retain guard.
#Now each element in load/other set and exit set is only a AST. compare them using _match_ast_sets. each element in store set is a two tuple,compare them using _match_ast_tuple_sets
#for each element in signature, we must find the same element in candidate. Only then we think they are match
#When compare two AST, we use function cmp_formula(). It conclude if two AST are "same". "Same" is not strict.
...
#conpare the number of each type . >= using _cnt_formula_type and _type_contains
#used in graph match
...
#topology compare generate candidates for signature
#used in do_match_sig. including pre_filter.
...
#find formulas of signature in candidate one by one using cmp_formula
...
#all sft should be included in candidate
...
#compare node by node,only when all nodes are matched return true. test for each node include: 1.classify_formulas    
#2._formula_match_load/_formula_match_store/_formula_match_exit 
...
#zz adaptation:
		match_insns_ignore_type = default_options.get('match_insns_ignore_type',False)
		if match_insns_ignore_type:
			cand_combine=combine_formulas(cand_loads,cand_stores)
			sig_combine=combine_formulas(sig_loads,sig_stores)
			if not _formula_match_load(sig_combine,cand_combine):
				return False
...
#1.graph_match(topology check) generate candidates(there may be more than one candidate which may corresponding to signature) which includes node-node mapping 
#2.semantic_match for each candidate
def match_sig():
